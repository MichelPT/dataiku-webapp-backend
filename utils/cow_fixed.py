...
class COW:
    ...
    def correlation(self, frameX, frameY):
        ...
        return float(correl) / float(prod)
    ...
    def resample(self, segX, N, beforeX=None, afterX=None):
        ...
        if (pX > N):  # subsample segX
            for i in range(N):
                ...
                if (idx > 0):
                    slope = (segX[idx] - segX[idx-1]) / \
                        (float)(Xabs[idx] - Xabs[idx-1])
                    # value for resampled segX at currentXabs
                    sampledSegX.append(
                        segX[idx-1] + slope * (currentXabs - Xabs[idx-1]))
    ...
    def setBounds(self):
        ...
        for i in range((self.n)):
            bX.append(i*nSegX)
            bY.append(i*nSegY)
        # last segment contains also remainder in nX/self.n
        bX.append(len(self.X))
    ...
    def forward_search(self):
        ...
        for j in range(jm, jM+1):  # loop over possible left bound position for first segment
            resampledSegX = self.resample(self.X[j:], len(
                segY), self.X[j-1], None)  # resample segX to size of segY
            # update  with correlation btw resampled segX and segY
            F[-1][j] = self.correlation(resampledSegX, segY)
        ...
        for i in bIdx[1:-2]:  # loop over segments
            ...
            for j in range(jm, jM+1):  # loop over possible left bound position for each segment
                ...
                correl_comp = self.max_correl_over_combinations(
                    F, U, i, j, segY)
        ...
        # last segment
        # maximum correlation and corresponding path for last position, (0,0)
        correl_comp = self.max_correl_over_combinations(
            F, U, 1, 0, self.Y[:bY[bIdx[-2]]])
    ...
    def max_correl_over_combinations(self, F, U, segIdX, position, segY):
        ...
        for j in range(segL - s, segL + s + 1):  # loop over possible segment lengths
            ...
            # examine if position was possible at previous iteration
            if ((prevj < len(F[0])) and F[segIdX][prevj] > float(0)):
                ...
                if (position > 0):  # all segment but last, r to l
                    resampledSegX = self.resample(self.X[position:prevj], len(
                        segY), self.X[position-1], self.X[prevj])
                else:  # last segment
                    resampledSegX = self.resample(
                        self.X[position:prevj], len(segY), self.X[0], self.X[prevj])
                ...
                corr_val = float(self.correlation(resampledSegX, segY))
                if F[segIdX][prevj] + corr_val > max_corr:
                    max_corr = F[segIdX][prevj] + corr_val
                    max_corr_path = list(U[segIdX][prevj]) + [position]
    ...
    def warp_sample_to_target(self):
        ...
        if fwdsrch is not False:
            path = fwdsrch[1][0][0]
            path = list(path)[::-1]   # pastikan list, bukan np.ndarray
        else:
            return
        ...
        for i in range(len(path)-2):
            ...
            resampledSegX = self.resample(
                segX, sY, self.X[(path[i+1]-1)], self.X[path[i+2]])
