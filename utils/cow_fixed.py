        ...
        for j in range(segL - s, segL + s + 1):  # loop over possible segment lengths
            ...
            if ((prevj < len(F[0])) and F[segIdX][prevj] > float(0)):
                ...
                if (F[segIdX][prevj] + self.correlation(resampledSegX, segY) > max_corr):
                    # update max_corr if current combination is improving cumulative correlation
                    max_corr = F[segIdX][prevj] + \
                        self.correlation(resampledSegX, segY)
                    max_corr_path = U[segIdX][prevj] + \
                        [position]  # update path accordingly
