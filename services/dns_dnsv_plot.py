...
def plot_dns_dnsv(df):
    """Create DNS-DNSV visualization plot"""
    df_marker = extract_markers_with_mean_depth(df)
    df_well_marker = df.copy()
    
    # Define plot sequence
    sequence = ['MARKER', 'GR', 'RHOB', 'NPHI', 'VSH', 'DNS', 'DNSV']
    ...
    for n_seq, col in plot_sequence.items():
        if col in ['GR', 'RHOB', 'NPHI', 'VSH', 'DNS', 'DNSV']:
