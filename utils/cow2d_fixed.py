...
# Assuming cow_fixed.py is in the same directory or in the Python path
...
class COW2D:
    ...
    # sample matrix, reference/target matrix ; sample matrix is 2D chromatogram to be arranged
    sample, target = [], []
    ...
    def __init__(self, sampleV=None, nbFram1=None, nbFram2=None, sl1=None, sl2=None, win1=None, win2=None):
        ...
        self.s1, self.s2 = int(sl1), int(sl2)
        self.sample = sampleV
        self.n1, self.n2 = int(nbFram1), int(nbFram2)
        self.L1, self.L2 = int(win1), int(win2)

        self.target = [[self.sample[i][j] for j in range(len(self.sample[0]))] for i in range(
            len(self.sample))]  # at first, target is sample
    ...
    def alignSampleToTargetDim2(self, r):
        ...
        align = cow.COW(sampleVect, targetVect, int(self.n2), int(self.s2))
