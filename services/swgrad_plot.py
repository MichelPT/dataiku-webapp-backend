...
def plot_swgrad(df):
    """Create SWGRAD visualization plot"""
    df_marker = extract_markers_with_mean_depth(df)
    df_well_marker = df.copy()
    
    # Define plot sequence
    sequence = ['MARKER', 'GR', 'RT', 'VSH', 'NPHI_RHOB', 'SWGRAD'] + [f'SWARRAY_{i}' for i in range(1, 26)]
    ...
    # Add ratios for SWARRAY columns
    for i in range(1, 26):
        ratio_plots[f'SWARRAY_{i}'] = 0.1
    ...
    for n_seq, col in plot_sequence.items():
        if col in ['GR', 'RT', 'VSH', 'SWGRAD'] or col.startswith('SWARRAY_'):
