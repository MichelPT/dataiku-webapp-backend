...
def process_dgsa_for_well(df_well: pd.DataFrame, params: dict, target_intervals: list, target_zones: list) -> pd.DataFrame:
    """
    Memproses DGSA sesuai dengan alur kerja Loglan (filter GR dinamis).
    """
    ...
    # Default window 100 data points
    window_size = int(params.get('SLIDING_WINDOW', 100))
    gr_caps = []

    df_filtered_gr = df_dgsa[(df_dgsa[gr_col] > 5) & (df_dgsa[gr_col] < 180)]
    ...
    if not gr_caps:
        ...
        df_dgsa['GR_MAX'] = 180.0
    else:
        gr_cap_df = pd.DataFrame(gr_caps).drop_duplicates(
            subset=['GR_DEP']).sort_values('GR_DEP').reset_index()
        df_dgsa['GR_MAX'] = np.interp(
            df_dgsa['DEPTH'], gr_cap_df['GR_DEP'], gr_cap_df['GR_CAP'])
    ...
    rhob_filter = (float(params.get('RHOBWAT_MIN', 2)),
                   float(params.get('RHOBWAT_MAX', 2.8)))
    coeffs = []

    for start in range(0, len(df_dgsa) - window_size, step):
        window = df_dgsa.iloc[start:start+window_size]

        mask_filter = (window[gr_col] < window['GR_MAX']) & \
                      (window[rhob_col] > rhob_filter[0]) & (
                          window[rhob_col] < rhob_filter[1])
    ...
    # --- LANGKAH 3: (SESUAI LOGLAN) HITUNG LOG DGSA FINAL ---
    print("Langkah 3: Menghitung log DGSA final...")
    df_dgsa['DGSA'] = df_dgsa.apply(
        lambda row: (
            _interpolate_coeffs(row['DEPTH'], coeff_df).dot(
                [1, 0.01 * row[gr_col],
                    (0.01 * row[gr_col])**2, (0.01 * row[gr_col])**3]
            )
        ),
        axis=1
    )
